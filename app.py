...
VIDEO_AUTO_MAX_DURATION_MS = 17000
VIDEO_MOMENT_INTERVAL_MS = 100
VIDEO_MOMENT_MAX_FRAMES = 160
VIDEO_AI_NATIVE_BATCH_SIZE = 6
VIDEO_AI_NATIVE_WORKERS = 3
VIDEO_AI_CALL_TIMEOUT_SECONDS = 60
...
<div class="live-camera-wrap">
  ...
  <div id="camera-recording-status" class="camera-recording-status" hidden>● 録画中 0:00 / 0:15</div>
...
export default function(component) {
  ...
  const updateRecordingClock = () => {
    ...
    recordingStatus.textContent = `● 録画中 ${mm}:${ss} / 0:15`;
  };
  ...
  const setCameraMenuBusy = (busy) => {
    cameraStartInFlight = Boolean(busy);
    if (startButton) startButton.disabled = Boolean(busy);
    if (videoStartButton) videoStartButton.disabled = Boolean(busy);
    if (modeSwitchButton) modeSwitchButton.disabled = Boolean(busy) || Boolean(mediaRecorder && mediaRecorder.state === 'recording');
  };
  ...
  const startCamera = async (mode = 'photo') => {
    ...
    setStatus(requestedMode === 'video' ? 'カメラとマイクを準備しています…' : 'カメラを準備しています…');
    let nextStream = null;
    try {
      ...
      if (componentDisposed || generation !== cameraStartGeneration) {
      ...
      try {
        ...
        // Chrome raises AbortError when a newer camera start supersedes play().
        // That is cancellation, not a camera permission failure.
        if (componentDisposed || generation !== cameraStartGeneration || playErr?.name === 'AbortError') return;
        throw playErr;
      }

      if (componentDisposed || generation !== cameraStartGeneration) return;
      shootButton.disabled = false;
      shootButton.textContent = requestedMode === 'video' ? '● 録画を開始' : '● 写真を撮る';
      showCameraActions();
      try { localStorage.setItem('tokyo_burari_last_camera_open_v1', String(Date.now())); } catch (_) {}
      setStatus(requestedMode === 'video' ? '動画は最大15秒です。録画後は自動保存・自動切り抜きを行います。' : '');
    } catch (err) {
      if (nextStream) {
        try { nextStream.getTracks().forEach((track) => track.stop()); } catch (_) {}
      }
      if (componentDisposed || generation !== cameraStartGeneration || err?.name === 'AbortError') return;
      console.error(err);
      releaseCurrentStream();
      ...
      if (generation === cameraStartGeneration && !componentDisposed) setCameraMenuBusy(false);
    }
  };
  ...
  const startVideoRecording = async () => {
    ...
    try {
      ...
      const pixels = Math.max(0, Number(settings.width || video.videoWidth || 0)) * Math.max(0, Number(settings.height || video.videoHeight || 0));
      const requestedVideoBitrate = pixels >= 1700000 ? 6000000 : (pixels >= 800000 ? 4500000 : 3000000);
      const options = {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          // Video is saved automatically. No review/save button is required.
          pendingMedia = null;
          if (stream) {
            try { stream.getTracks().forEach((track) => track.stop()); } catch (_) {}
            stream = null;
          }
          if (video) { try { video.pause(); } catch (_) {} video.srcObject = null; video.hidden = true; }
  ...
  const retryPendingMedia = async () => {
    ...
    if ((source === 'camera' || source === 'video_camera') && stream && stream.getTracks().some((track) => track.readyState === 'live')) {
      ...
      try { await video.play(); } catch (err) { if (err?.name !== 'AbortError') throw err; }
      shootButton.disabled = false;
      showCameraActions();
      return;
    }
...
def _decode_browser_media_data_url(data_url, expected_kind=None):
    """Decode component media and validate by payload signature, not MIME label alone."""
    ...
    mime_type = _sniff_media_mime(raw) or declared_mime
    if expected_kind == "image" and not str(mime_type).startswith("image/"):
        raise ValueError("撮影画像の形式を判定できませんでした。")
    if expected_kind == "video" and not str(mime_type).startswith("video/"):
...
def decode_camera_video_data_url(data_url):
    return _decode_browser_media_data_url(data_url, "video")
...
def openai_client():
    ...
    return OpenAI(
        ...
        timeout=float(VIDEO_AI_CALL_TIMEOUT_SECONDS),
        max_retries=1,
    )
...
def photo_display_url(photo, signed_map=None, max_px=420, quality=76):
    ...
    try:
        ...
        pass
    try:
        return image_data_url(download_photo(path))
    except Exception:
        return ""
...
def upload_photo(trip_id, image_bytes, location=None, captured_at=None, capture_source="camera", extra_reflection=None, preserve_original=False):
    ...
    raw_input = bytes(image_bytes or b"")
    if not raw_input:
        raise ValueError("写真データが空です。")
    if preserve_original:
        stored_bytes = raw_input
        content_type = _sniff_media_mime(stored_bytes) or "image/png"
        extension = ".png" if content_type == "image/png" else (".webp" if content_type == "image/webp" else ".jpg")
    else:
        stored_bytes = normalize_photo(raw_input)
        if not stored_bytes:
            raise ValueError("写真データが空です。")
...
def upload_video(
    ...
    if duration_value > 17000:
        raise ValueError("動画が15秒を大きく超えています。15秒程度で撮影してください。")
    if len(video_bytes) > 18 * 1024 * 1024:
        raise ValueError("動画データが18MBを超えています。15秒程度で撮影してください。")
...
def _video_native_selection_schema_v146(count):
...
def _video_native_ai_prompt_v146(keep_count, final=False):
    if final:
        return (
            "15秒程度の動画から、最終的に写真として自動保存する9枚を選んでください。"
            "候補画像はすべて元動画から元解像度のまま切り出した画像です。"
            "自然な笑顔・表情、決定的な動作、被写体の魅力、構図、光、ピント、ブレの少なさを総合評価してください。"
            "同じ瞬間に近い似た画像を重複させず、動画全体を代表する強い9枚にしてください。"
            "内面の感情は断定せず、画像から見える表情や動きだけを評価してください。"
            "rank=1をAI BESTとして、rank=1〜9を重複なく返してください。"
        )
    return (
        f"動画の連続フレームから、この組の中で特に写真として残したい{int(keep_count)}枚を選んでください。"
        "候補は元動画から0.1秒間隔・元解像度のまま切り出した画像です。"
        "映え、自然な表情、決定的瞬間、被写体の魅力、構図、光、ピント、手ぶれの少なさを優先してください。"
        "似た連続フレームでは最も良い1枚を優先してください。"
        "rankは1から順にし、frame_idを候補ラベルどおり正確に返してください。"
    )


def _select_native_video_batch_v146(records, keep_count, stage_name, final=False):
    records = list(records or [])
    if not records:
        return []
    keep_count = max(1, min(int(keep_count), len(records)))
    prompt = _video_native_ai_prompt_v146(keep_count, final=final)
    schema = _video_native_selection_schema_v146(keep_count)
    image_items = []
    for record in records:
        raw = Path(record["path"]).read_bytes()
        image_items.append(
            (f"{record['frame_id']} / {int(record['timestamp_ms']) / 1000:.1f}秒", raw)
        )
    last_exc = None
    for attempt in range(2):
        try:
            result = ask_json_with_images(
                prompt,
                image_items,
                f"video_native_{stage_name}_{attempt + 1}",
                schema,
                max_output_tokens=1500 if final else 800,
            )
            break
        except Exception as exc:
            last_exc = exc
            result = None
    if result is None:
        raise RuntimeError(f"AI画像選定に失敗しました: {last_exc}")

    by_id = {str(record.get("frame_id") or ""): record for record in records}
    ranked = sorted(
    ...
    for item in ranked:
        ...
        if not record or frame_id in used:
            continue
        merged = dict(record)
        merged.update(
            {
        ...
        used.add(frame_id)
        if len(output) >= keep_count:
            break
    if len(output) != keep_count:
        raise RuntimeError("AIが必要枚数のフレームを返しませんでした。")
    return output


def _run_native_video_stage_v146(records, batch_size, keep_per_batch, stage_name):
    records = list(records or [])
    if not records:
        return []
    batches = [records[i:i + int(batch_size)] for i in range(0, len(records), int(batch_size))]
    workers = max(1, min(int(VIDEO_AI_NATIVE_WORKERS), len(batches)))
    selected = []
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix=f"burari-{stage_name}") as executor:
        future_map = {}
        for index, batch in enumerate(batches, start=1):
            keep = min(int(keep_per_batch), len(batch))
            future = executor.submit(
                _select_native_video_batch_v146,
                batch,
                keep,
                f"{stage_name}_{index}",
                False,
            )
            future_map[future] = index
        for future in as_completed(future_map):
            selected.extend(future.result())
    selected.sort(key=lambda record: int(record.get("timestamp_ms") or 0))
    return selected


def _rollback_auto_saved_moments_v146(saved_rows):
    rows = [row for row in (saved_rows or []) if isinstance(row, dict)]
    if not rows:
        return
    client = supabase_client()
    ...
    for row in rows:
        try:
            client.table(PHOTO_TABLE).delete().eq("id", row.get("id")).eq("family_key", current_family_key()).eq("member_key", current_member_key()).execute()
        except Exception:
            pass
    if paths:
        try:
...
def process_video_good_moments_v146(video_photo, video_bytes, mime_type="video/webm"):
    """Automatic quality-preserving video pipeline.

    The original recorded video bytes are the single source of truth. Every 0.1-second
    candidate is decoded at native dimensions to lossless PNG. No candidate is resized
    or JPEG-recompressed before AI inspection. The nine AI-selected PNG files are then
    inserted directly into the normal photo collection without recompression.
    """
    if not isinstance(video_photo, dict) or not video_photo.get("id") or not photo_is_video(video_photo):
        raise ValueError("自動切り抜き対象の動画が見つかりません。")
    if not video_bytes:
        raise ValueError("元動画データがありません。")
    ffmpeg = _ffmpeg_executable_v146()
    if not ffmpeg:
        raise RuntimeError("元動画から画像を作るためのffmpegを利用できません。")

    duration_ms = max(1, int(photo_media_metadata(video_photo).get("video_duration_ms") or 0))
    duration_ms = min(duration_ms or 15000, VIDEO_AUTO_MAX_DURATION_MS)
    target_count = max(9, min(VIDEO_MOMENT_MAX_FRAMES, int((duration_ms + VIDEO_MOMENT_INTERVAL_MS - 1) // VIDEO_MOMENT_INTERVAL_MS)))
    clean_mime = str(mime_type or "video/webm").lower()
    suffix = ".mp4" if "mp4" in clean_mime or "quicktime" in clean_mime else ".webm"
    ...
    reflection["ai_selection"] = {
        ...
        "pipeline": "native_lossless_v146",
        "items": [],
    }
    _write_photo_reflection(video_photo["id"], reflection)

    saved_rows = []
    with tempfile.TemporaryDirectory(prefix="burari-v146-") as tmpdir:
        input_path = Path(tmpdir) / f"original{suffix}"
        frames_dir = Path(tmpdir) / "frames"
        frames_dir.mkdir(parents=True, exist_ok=True)
        input_path.write_bytes(video_bytes)
        output_pattern = str(frames_dir / "native_%03d.png")
        fps = 1000.0 / float(VIDEO_MOMENT_INTERVAL_MS)
        command = [
            ffmpeg,
            "-nostdin",
            "-hide_banner",
        ...
        try:
            ...
            raise RuntimeError("元動画からのフレーム作成が60秒で時間切れになりました。") from exc

        frame_paths = sorted(frames_dir.glob("native_*.png"))[:target_count]
        if completed.returncode != 0 and not frame_paths:
            detail = completed.stderr.decode("utf-8", errors="ignore").strip().replace("\n", " ")[:240]
            raise RuntimeError("元動画からフレームを作成できませんでした。" + (f" {detail}" if detail else ""))
        if len(frame_paths) < 9:
            raise RuntimeError("元動画から9枚以上の有効なフレームを作成できませんでした。")

        records = [
            {
                "frame_id": f"F{index:03d}",
                "timestamp_ms": (index - 1) * VIDEO_MOMENT_INTERVAL_MS,
                "path": str(frame_path),
            }
        ...
        reflection = dict(photo_media_metadata(video_photo))
        current = reflection.get("ai_selection") or {}
        if not isinstance(current, dict):
            current = {}
        current.update({"status": "processing", "stage": "ai_first_pass", "candidate_count": len(records), "updated_at": now_jst().isoformat()})
        reflection["ai_selection"] = current
        _write_photo_reflection(video_photo["id"], reflection)

        working = records
        narrowing_round = 1
        while len(working) > 18:
            working = _run_native_video_stage_v146(
                working, VIDEO_AI_NATIVE_BATCH_SIZE, 2, f"narrow_{narrowing_round}"
            )
            narrowing_round += 1
        if len(working) > 12:
            working = _run_native_video_stage_v146(working, 3, 2, "semifinal")
        if len(working) < 9:
            raise RuntimeError("AI最終選定に必要な候補数を確保できませんでした。")
        finalists = _select_native_video_batch_v146(working, 9, "final", final=True)

        location = reflection.get("location") if isinstance(reflection.get("location"), dict) else {}
        items = []
        try:
            for rank, selected in enumerate(finalists, start=1):
                raw_png = Path(selected["path"]).read_bytes()
                saved = upload_photo(
                    video_photo.get("trip_id"),
                    raw_png,
                    location=location,
                    captured_at=_selection_capture_time(video_photo, selected.get("timestamp_ms")),
                    capture_source="video_auto_moment",
                    preserve_original=True,
                    extra_reflection={
                        "source_video_photo_id": str(video_photo.get("id") or ""),
                        "source_selection_rank": rank,
                        "source_selection_timestamp_ms": int(selected.get("timestamp_ms") or 0),
                        ...
                        "quality_pipeline": "native_lossless_v146",
                    },
                )
                if not isinstance(saved, dict) or not saved.get("id"):
                    raise RuntimeError("切り抜き写真を自動保存できませんでした。")
                saved_rows.append(saved)
                items.append(
                    {
                        "rank": rank,
                        "storage_path": str(saved.get("storage_path") or ""),
                        "frame_id": str(selected.get("frame_id") or ""),
                        "timestamp_ms": int(selected.get("timestamp_ms") or 0),
                        ...
                        "ai_best": rank == 1,
                        "saved_photo_id": str(saved.get("id") or ""),
                        "auto_saved": True,
                        "output_source": "original_video_native_png_v146",
                    }
                )
        except Exception:
            _rollback_auto_saved_moments_v146(saved_rows)
            raise
...
def choose_video_ai_frames(frame_items):
    """Ask vision AI for nine diverse best stills; no category quota is imposed."""
    frames = list(frame_items or [])
    if len(frames) < 9:
        raise ValueError("AIセレクション用の候補画像が9枚未満です。")
    ...
    prompt = (
        "動画から静止画9枚を選ぶフォトセレクターです。候補フレームを時刻順に渡します。\n"
        "カテゴリ別に3枚ずつ等の枚数制限は一切ありません。9枚すべてを総合評価で選んでください。\n"
        "人物・生物では、笑顔・驚き・真剣さなど画像から明確に見える表情、印象的なしぐさ、躍動感、"
        "顔や全身の見え方を重視します。内面の感情は断定せず、見た目の表情だけを評価してください。\n"
        "物・乗り物・風景では、被写体がきれいに見える瞬間、動きのタイミング、ピント、ブレ、露出、色、"
        "構図、遮蔽物の少なさを重視してください。\n"
        "どの被写体でも、写真としての完成度と『この動画を代表する印象的な瞬間』を評価してください。\n"
        "重要: ほぼ同じ場面・同じ表情の近接フレームを複数選ばず、動画全体の異なる時点や場面がなるべく"
        "含まれるようにしてください。ただし質を下げてまで均等配分する必要はありません。\n"
        "rank=1をAI BESTとし、rank=1〜9を重複なく返してください。reasonは日本語で短く具体的にしてください。"
    )
    image_items = [
        (f"{frame['frame_id']} / {int(frame['timestamp_ms']) / 1000:.1f}秒", frame["ai_bytes"])
        for frame in frames
    ]
    try:
        result = ask_json_with_images(
        ...
        # Some model/provider limits are more conservative with many images. Retry
        # with 12 temporally spread candidates rather than failing the saved video.
        if len(frames) <= 12:
            raise
    ...
    for item in ranked:
        ...
        # Reject only near-identical frames here. Broader semantic diversity is
        # handled by the AI prompt so different expressions in one scene can remain.
    ...
    # A malformed response or an almost-identical pick must not leave an 8-cell
    # grid. Fill from unused frames, prioritizing the time point furthest from the
    # already selected set while still rejecting exact-looking duplicates.
    while len(selected) < 9:
        candidates = []
        selected_times = [int(x["frame"].get("timestamp_ms") or 0) for x in selected]
        for frame in frames:
            frame_id = str(frame.get("frame_id") or "")
            if frame_id in used_ids:
                continue
            frame_hash = _image_dhash64(frame.get("image_bytes"))
            if any(_hamming64(frame_hash, existing) <= 1 for existing in used_hashes):
                continue
            timestamp = int(frame.get("timestamp_ms") or 0)
            distance = min((abs(timestamp - t) for t in selected_times), default=10**9)
            candidates.append((distance, timestamp, frame, frame_hash))
        if not candidates:
            break
        _, _, frame, frame_hash = max(candidates, key=lambda x: (x[0], x[1]))
        selected.append(
            {
                "frame": frame,
                "score": 0,
                "primary_quality": "other",
                "reason": "動画全体の違う場面を残すため選択",
                "hash": frame_hash,
            }
        )
    ...
    if len(selected) < 9:
        # Final safety fallback for very short/static videos: allow visually similar
        # frames only when there are no nine distinct-looking candidates at all.
        for frame in frames:
            frame_id = str(frame.get("frame_id") or "")
            if frame_id in used_ids:
                continue
...
def render_recent_camera_photo_comment(trip):
    ...
    if is_video:
        render_video_ai_selection(photo, key_prefix="recent_video_selection", allow_save=True)
    ...
    if isinstance(video_payload, dict) and video_payload.get("data_url") and video_payload.get("poster_data_url"):
        try:
            ...
            if st.session_state.get(digest_key) != digest:
                ...
                with st.spinner("元動画をそのまま保存しています…"):
                ...
                if isinstance(saved_video, dict) and saved_video.get("id"):
                    try:
                        with st.spinner("元動画から0.1秒ごとの高画質画像を作り、いい瞬間を自動選定・保存しています…"):
                            saved_video, auto_saved_rows = process_video_good_moments_v146(
                                saved_video,
                                video_raw,
                                mime_type=mime_type or video_payload.get("mime_type"),
                            )
                        ai_selection_ok = len(auto_saved_rows) == 9
                    except Exception as selection_exc:
                        mark_video_ai_selection_error(saved_video, selection_exc)

                st.session_state[digest_key] = digest
                if auto_saved_rows:
                    st.session_state[f"_camera_recent_photo_{trip['id']}"] = auto_saved_rows[0].get("id")
                ...
                try:
                    ...
                    previous_count = 0
                st.session_state["_home_today_photo_count"] = previous_count + len(auto_saved_rows)
                ...
                st.session_state["_camera_notice"] = (
                    "動画を保存し、いい瞬間9枚を元動画画質のまま自動保存しました。"
                    if ai_selection_ok else
                    "動画は保存しましたが、いい瞬間の自動保存を完了できませんでした。"
                )
                st.rerun()
        except Exception as exc:
...
def page_settings():
    ...
    if quota_bytes > 0:
        try:
            ...
            st.caption(f"残り：{format_storage_size(remaining_bytes)}。AIセレクションの静止画はこの動画容量には含めません。")
